...
class ChemprotDataset(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        ...
        BigBioConfig(
            name="chemprot_shared_task_eval_bigbio_kb",
            version=BIGBIO_VERSION,
            description="chemprot BigBio schema with only the relation types that were used in the shared task evaluation",
            schema="bigbio_kb",
            subset_id="chemprot_shared_task_eval",
        ),
        BigBioConfig(
    ...
    def _generate_examples(
        ...
        if self.config.schema == "source":
            ...
            if self.config.subset_id == "chemprot_full":
                ...
                relations = self._get_relations_gs(
                    os.path.join(filepath, gold_standard_file), is_mapped=True
                )
            else:
                raise NotImplementedError(f"Unsupported subset with id {self.config.subset_id}")

...
class DDICorpusDataset(datasets.GeneratorBasedBuilder):
    ...
    BUILDER_CONFIGS = [
        ...
        BigBioConfig(
            name="ddi_corpus_re_bigbio_kb",
            version=BIGBIO_VERSION,
            description="DDI Corpus BigBio schema",
            schema="bigbio_kb",
            subset_id="ddi_corpus_re",
        ),
    ]
    ...
    def _generate_examples(self, filepath: str, split: str) -> Tuple[int, Dict]:
        ...
        for subdir, file in files:
            ...
            if self.config.schema == "source":
                example = self._to_source_example(brat_example)
                yield example["document_id"], example

            elif self.config.schema == "bigbio_kb":
                example = brat_parse_to_bigbio_kb(brat_example)
                example["id"] = example["document_id"]

                if self.config.subset_id == "ddi_corpus":
                    yield example["document_id"], example

                elif self.config.subset_id == "ddi_corpus_re":
                    if (
                            (split == "test" and len(example["relations"]) > 0) or
                            (split == "train" and len(example["entities"]) >= 2)
                    ):
                        yield example["document_id"], example
